...
```{r innit}
setwd("C:/Users/Quesada/Downloads/archive")
dt <- data.table::fread("pmsm_temperature_data.csv") # 0.5 secs between rows
```
...
```{r fit_network}
model_net <- dbnR::learn_dbn_struc(dt_train, size, method = "psoho", f_dt = f_dt_train, n_it = 50, n_ind = 100)
model_fit <- dbnR::fit_dbn_params(model_net, f_dt_train)
model <- mtDBN::mtDBN$new()
model$fit_model(dt_train, size, method = "psoho", obj_var = "pm", min_ind = 80, max_depth = 6, f_dt = f_dt_train, n_it = 50, n_ind = 100)
...
res_net <- dbnR::forecast_ts(f_dt_test, model_fit, 3, obj_vars = obj_vars, ini = ini, len = len, prov_ev = ev_vars)
