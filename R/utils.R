...
launch_hybrid_model <- function(dt_train, f_dt_train, f_dt_test, id_var, obj_vars, mv, homogen, pred_len, method, min_ind, max_depth,
...
train_test_iteration <- function(dt, id_var, test_id, obj_vars, 
  ...
  res_matrix[1,] <- launch_single_model(f_dt_train, f_dt_test, id_var, obj_vars, pred_len, method, min_ind, max_depth,
                                      n_it, n_ind, gb_cte, lb_cte, cte, r_probs, v_probs)
  res_matrix[2,] <- launch_hybrid_model(dt_train, f_dt_train, f_dt_test, id_var, obj_vars, F, T, pred_len, method, 
                                        min_ind, max_depth, n_it, n_ind, gb_cte, lb_cte, cte, r_probs, v_probs, prune_val)
  res_matrix[3,] <- launch_hybrid_model(dt_train, f_dt_train, f_dt_test, id_var, obj_vars, T, T, pred_len, method,
                                        min_ind, max_depth, n_it, n_ind, gb_cte, lb_cte, cte, r_probs, v_probs, prune_val)
  res_matrix[4,] <- launch_hybrid_model(dt_train, f_dt_train, f_dt_test, id_var, obj_vars, T, F, pred_len, method,
                                        min_ind, max_depth, n_it, n_ind, gb_cte, lb_cte, cte, r_probs, v_probs, prune_val)
  res_matrix[5,] <- launch_hybrid_model(dt_train, f_dt_train, f_dt_test, id_var, obj_vars, F, F, pred_len, method,
                                        min_ind, max_depth, n_it, n_ind, gb_cte, lb_cte, cte, r_probs, v_probs, prune_val)
